...
parseStatementTests =
           TestList [ parseStatementT "a = b[1];" ~?= Assign [LValueComp "a" []] (Var [LValueComp "b" [IntLit 1]])
                    ...
                    , parseStatementT "if (i == 1) print();" ~?= CondStructStmt [CondStmt (Binary Equal (Var [LValueComp "i" []]) (IntLit 1)) (FunctionCallS (FunctionCallE Nothing False (FuncName [] "print") []))] Nothing
                    , parseStatementT "if (i == 1) if (j == 0) print();" ~?= CondStructStmt [CondStmt (Binary Equal (Var [LValueComp "i" []]) (IntLit 1)) (CondStructStmt [CondStmt (Binary Equal (Var [LValueComp "j" []]) (IntLit 0)) (FunctionCallS (FunctionCallE Nothing False (FuncName [] "print") []))] Nothing)] Nothing
